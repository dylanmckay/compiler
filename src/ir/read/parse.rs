    ...
    where I: Iterator<Item=char>
{
    ...
    fn parse_block_helper(&mut self, label: String) -> Result<Block> {
        let mut block = Block::empty(label);

        self.resolve.give(&mut block);
        block.extend(try!(self.parse_block_values()));
