...
pub fn root_value(value: &Value,
                  printer: &mut Printer,
                  fmt: &mut fmt::Formatter) -> fmt::Result {
    root_expression(&value.node, printer, fmt)
}
...
pub fn value(value: &Value,
             printer: &mut Printer,
             fmt: &mut fmt::Formatter) -> fmt::Result {
    expression::expression(&value.node, printer, fmt)
}

pub fn plain_value(value: &Value,
                   printer: &mut Printer,
                   fmt: &mut fmt::Formatter) -> fmt::Result {
    expression::plain(&value.node, printer, fmt)
}
...
pub mod expression
{
    ...
    pub mod instruction
    {
        ...
        pub fn call(inst: &instruction::Call,
                    fmt: &mut fmt::Formatter) -> fmt::Result {
            let func = if let Expression::FunctionRef(ref f) = inst.target().node  {
                f
            } else {
